...
Setup also creates two files of its own: `~/.config/hypr/pointer_scroll.lua` (generated
Hyprland settings) and `~/.config/hypr/pointer_scroll.json` (your slider values). It never replaces a
file it didn't create: if either path already exists and wasn't made by this widget, setup refuses
and changes nothing, and the panel says which file is in the way. (Its own files are recognised by a
marker: the `.lua` starts with a `-- GENERATED by omarchy-pointer-scroll` line and the `.json` has a
`"_generator"` field.)
...
settings file is kept. It only deletes `pointer_scroll.lua` if this widget created it; if the file
is someone else's, uninstall leaves both it and the `require` line alone.
